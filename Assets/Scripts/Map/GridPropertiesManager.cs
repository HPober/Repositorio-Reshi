...
public class GridPropertiesManager : SingletonMonobehaviour<GridPropertiesManager>, ISaveable
{
    ...
    [SerializeField] private Tile[] ground = null;
    ...
    public void ClearCropInPositionIfExists(Vector2Int gridPos)
    {
        Crop[] cropArray;
        cropArray = FindObjectsOfType<Crop>();
        ...
        foreach (Crop crop in cropArray)
        {
            if(crop.cropGridPosition == gridPos)
            {
                Destroy(crop.gameObject);
                break;
            }
    ...
    public void ClearDugGround(GridPropertyDetails gridPropertyDetails)
    {
        // Check if the ground is already dugged
        if (gridPropertyDetails.daysSinceDug == -1)
        {
            Vector3Int pos = new Vector3Int(gridPropertyDetails.gridX, gridPropertyDetails.gridY, 0);
            groundDecoration1.SetTile(pos, null);
            groundDecoration2.SetTile(pos, null);
